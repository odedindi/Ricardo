...
export const fetchSearchResults = async (searchText: string):Promise<SearchResponse> => {
...
const fetchUserData = async (userId: Id):Promise<GetUserResponse> => {
...
export const fetchArticleDetails = async (articleId: Id):Promise<ChosenArticle> => {
    ...
    if (respond.ok) {
        ...
        const articleDataAndUser:ChosenArticle = { article: data, user: user }
